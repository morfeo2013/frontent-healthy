...
export default function Login() {
    ...
    const login = async (e) => {
        ...
        const respuesta = await Axios.post('https://ganohealthy.herokuapp.com/ingresar', usuario)/* 
        ...
        if (mensaje === 'Ingresate login valido desde el backend') {
            ...
            const nombre=respuesta.data.nombre /* despuesde recibir el nombre para mostrarl oen el navvar */
            ...
            setTimeout(() => {
                if(admin===id){window.location.href = '/visualAdmin'}/*vista para el admin */
                else{window.location.href = '/VistaProductosUsuario'}/*vista para el ususario */
            }, 1500)
                        ...
                        <form onSubmit={login}>
                            <div className="form-group mt-2">
                                <input type="email" name="correo" className="form-control"
                                    ...
                                    placeholder="Correo" required autoFocus onChange={e => setCorreo(e.target.value)}
                                    value={correo} />
